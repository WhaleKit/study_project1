...
#ifdef LOGGING_ENABLED
{
    ...
    /*static верните если захотите повеселиться*/auto finish = [&]()mutable-> bool
        {
            bool grounded = false;
...
bool LineSegmentsIntersects (Vector2f const A_arg, Vector2f const B_arg, Vector2f const C_arg, Vector2f const D_arg)
{
    ...
    {
        return false; //небольшая оптимизация
    }
    ...
    //уравн. прямой y = t*x+h;
    //находим коэффициенты в уравн. прямой
    float abT, abH;
    if (!abIsVertical)
    {
    ...
    if ( (!abIsVertical) && (!cdIsVertical) )
    {
        if ( abT != cdT )
        {
            ...
            //если точка пересечения прямых находится в пределах отрезков, то они пересекаются
        ...
        {//если отрезки параллельны, я считаю, что они епресекаются, если пересекаются множества их точек
            return (abH == cdH)
